...
LIST_OF_COLLECTIONS = ["funcionarios", "pontos"]
logger = logging.getLogger(name="Sistema_Pontos_Funcionarios_MongoDB")
...
def extract_and_insert():
    ...
    for collection in LIST_OF_COLLECTIONS:
        ...
        if collection == "pontos":
            df["data_ponto"] = df["data_ponto"].dt.strftime("%m-%d-%Y")
            df["hora_entrada"] = df["hora_entrada"].dt.strftime("%H:%M:%S")
            df["hora_saida"] = df["hora_saida"].dt.strftime("%H:%M:%S")
